...
function getSellers($bookTitle) {
    global $db;
    $query = "SELECT a.username, r.rating, a.profile_image, b.book_id, ub.book_condition FROM account a, book b, uploaded_books ub, review r WHERE a.account_id = ub.account_id AND b.book_id = ub.book_id AND r.account_id = a.account_id AND  b.title = :bookTitle GROUP BY a.username;";
    $statement = $db->prepare($query);
    $statement->bindValue(":bookTitle", $bookTitle);
    $statement->execute();
